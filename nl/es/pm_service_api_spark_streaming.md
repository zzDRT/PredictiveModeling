...
**Nota**: Esta función está actualmente en versión beta y solo está disponible para su uso con Spark MLlib.

**Nombre de caso de ejemplo**: Análisis de sentimiento. 

**Descripción del caso de ejemplo**: Una agencia de marketing quiere saber la opinión general sobre un determinado tema. La agencia quiere que desarrollemos un modelo que clasifique una determinada declaración como POSITIVA o NEGATIVA. Un científico de los datos prepara un modelo de predicción y lo comparte con el cliente (el desarrollador). La tarea del cliente consiste en desplegar el modelo y generar análisis predictivos mediante la realización de solicitudes de puntuaciones sobre el modelo desplegado.
...
2. En la sección Modelos de ejemplo, busque el mosaico Predicción de opiniones y pulse el botón Añadir modelo (+).

Ahora verá el modelo de predicción de opiniones de ejemplo en la lista de modelos disponibles en el separador Modelos.

## Generación de una señal de acceso
...
Para utilizar una llamada de API REST para crear un despliegue en modalidad continua de su modelo de predicción, especifique los siguientes detalles:

*  La señal de acceso creada en el paso anterior

*  Credenciales del servicio Spark, que encontrará en el separador Credenciales de servicio del panel de control del servicio Bluemix Spark. Antes de realizar la solicitud de despliegue, las credenciales de Spark se deben decodificar como base64 y se deben pasar en la cabecera de una solicitud curl como X-Spark-Service-Instance.
   ...
   En el sistema operativo macOS, utilice el siguiente mandato: 
...
{: codeblock}
   ...
   En sistemas operativos Microsoft Windows o Linux, debe utilizar el parámetro `--wrap=0` con el mandato `base64` para realizar la codificación base64: 
...
*  Detalles del tema IBM Message Hub que se utilizarán como entrada (tuits) para el modelo y almacenamiento para la salida del modelo (resultados de la predicción).

*  Utilice el siguiente punto final para crear un despliegue: 
   
   ```
   /v2/published_models/{published_model_id}/deployments
   ```
   
   El punto final que se necesita para crear un despliegue en línea está disponible en el panel de control de WML -> Detalles de modelo -> URL. Tenga en cuenta que puede encontrar el valor de published_model_id en el panel de control de IBM Watson Machine Learning, pulse Modelo -> Ver detalles y copie el valor en el campo ID y obtenga su "instance_id" de las credenciales VCAP de la instancia de Watson Machine Learning. 
...
**Nota**: También puede utilizar el panel de control para crear un despliegue en modalidad continua. Debe facilitar las tres entradas siguientes:

*  Detalles del archivo de origen que utilizará el modelo. El archivo de origen debe estar ya cargado y listo para ingestión (IBM Message Hub).

*  Detalles del archivo de origen, en el que se guardarán los resultados. El origen de la salida ya debe estar creado y listo para recibir los resultados a medida que el modelo procesa los datos procedentes del origen de la entrada.

*  Credenciales del servicio Spark.
...
Puede consultar los detalles de su despliegue en modalidad continua con una solicitud GET utilizando el campo Location que se devuelve como resultado del despliegue. Consulte la siguiente información.
...
## Detención de un despliegue en modalidad continua

De nuevo, utilizando el campo Location del resultado de un despliegue puede detener fácilmente un despliegue en modalidad continua del siguiente modo.
...
## Inicio de un despliegue en modalidad continua

Puede iniciar fácilmente un despliegue en modalidad continua del siguiente modo.
...
## Supresión de un despliegue en modalidad continua

Puede suprimir un despliegue en modalidad continua si ya no lo necesita.
